...
# 	perturb = perturb.reshape(3, 224, 224)
	...
	diff = np.transpose(diff.reshape(224, 224,3), (2, 0, 1))
...
# 	sample_2 = sample_2.reshape(3, 224, 224)
	sample_1 = np.transpose(sample_1.reshape(224, 224,3), (2, 0, 1))
	sample_2 = np.transpose(sample_2.reshape(224, 224,3), (2, 0, 1))
	diff = []
...
def boundary_attack():
	...
	l2thresh = 0.05 * np.sqrt(224*224)


	imagenet_path = '../imagenetval'

	provider = robustml.provider.ImageNet(imagenet_path, (224,224,3))

	start = 100
	end = 180
	hardsamples = [103,105,106,108,110,112,113,115,117,121,122,126,130,137,138,163,164,166,169,172,173,174,178]
	wrongexample = 0
	totalImages = 0
	succImages = 0
	classifier = ResNet50(weights='imagenet')
	for i in range(start,end):
		if i not in hardsamples:
			continue
		inputs, targets = provider[i]
		inputs = inputs.reshape(1,224,224,3)
		print('evaluating %d of [%d, %d]' % (i, start, end))
		sys.stdout.flush()
		logits = classifier.predict(preprocess_input(inputs* 255.0))
...
# 	print(wrongexample)
		...
		initial_sample = preprocess_input(inputs * 255.0)
		target_sample = preprocess_input(target_x.reshape(1,224,224,3) * 255.0)
...
# 		target_sample = preprocess('images/original/bad_joke_eel.png')
		...
		for attack_step in range(10000):
