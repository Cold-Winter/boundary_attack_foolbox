...
def orthogonal_perturbation(delta, prev_sample, target_sample):
	...
	perturb /= get_inf_diff(perturb, np.zeros_like(perturb))
...
def boundary_attack():
	...
	real_logits = model.model(input_xs)
...
# 		target_class = np.argmax(model.outlogits(target_sample.reshape(1,32,32,3)/255))
		attack_class = np.argmax(sess.run(real_logits, feed_dict={input_xs: initial_sample.reshape(1,32,32,3)}))
		target_class = np.argmax(sess.run(real_logits, feed_dict={input_xs: target_sample.reshape(1,32,32,3)}))
...
# 			prediction =  model.outlogits(trial_sample.reshape(1, 32, 32, 3)/255)
			prediction = sess.run(real_logits, feed_dict={input_xs: trial_sample.reshape(1,32,32,3)})
			n_calls += 1
...
# 				predictions = model.outlogits(np.array(trial_samples).reshape(-1, 32, 32, 3)/255)
				predictions = sess.run(real_logits, feed_dict={input_xs: np.array(trial_samples).reshape(-1,32,32,3)})
				n_calls += 10
